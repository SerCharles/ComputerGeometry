...
int main()
{
    ...
    double biggest = 2000000000000.0;
    ...
    for(int i = 1; i <= n; i ++)
    {
        ...
        for(int j = 1; j <= segments; j ++)
        {
            ...
            int y2 = 0;
            scanf("%d %d %d", &x, &y1, &y2);
            //Turn the segments into their dual halfspaces
            double a = double(x);
            double b_up = -double(min(y1, y2));
            double b_down = -double(max(y1, y2));
            double y_down_left = a * x_left + b_down;
            double y_down_right = a * x_right + b_down;
            double y_up_left = a * x_left + b_up;
            double y_up_right = a * x_right + b_up;
            Point down_left = Point(x_left, y_down_left);
            Point down_right = Point(x_right, y_down_right);
            Point up_left = Point(x_left, y_up_left);
            Point up_right = Point(x_right, y_up_right);     
            HalfSpace down = HalfSpace(down_left, down_right);
            HalfSpace up = HalfSpace(up_right, up_left);
